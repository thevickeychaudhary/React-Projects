import { Box, Flex, Grid, Heading, Icon, Text, position, useToast } from "@chakra-ui/react";
...
export const Deatils = () => {
    ...
    useEffect(() => {
        ...
        console.log("Initial weather data from session storage:", weather);
        
        // Clear old cached data if it exists (to prevent using expired API key data)
        if (weather && weather.weatherData && weather.weatherData.cod === 401) {
            console.log("Clearing expired API key data from cache");
            sessionStorage.removeItem("weather");
            weather = null;
        }
        
        if (!weather) {
            console.log("No cached weather data, fetching from location...");
        ...
        <>
            <Box maxW={'1400px'} m={'20px auto 5px'} p={'20px'} minH={'550px'}>
                ...
                <div className="tagLine" style={{marginTop: "20px", position: "sticky", bottom: "0", zIndex: "19", backgroundColor: "white"}}>
                    <p id="madeByAditya">Made by Vicky Choudhary</p>
                </div>
            </Box >
        </>
    );
